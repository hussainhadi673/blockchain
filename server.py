...
#w3 = #w3storage.API(token='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDk2NTE0ZjJhMDk4OTY1NjM1OGQ2NzM3M0VjN2E4MWNlZUI5RjFiNjAiLCJpc3MiOiJ3ZWIzLXN0b3JhZ2UiLCJpYXQiOjE2ODM3MzQ1MDA1NjIsIm5hbWUiOiJoYWRpLXRva2VuIn0.NgeorjO2Gc8SNpXcElCIpbFvlQX8sjzbfSOHzCe_FZE')
#some_uploads = w3.user_uploads(size=25)


projectId = "2PdY2PZ3GcaGW9NMpB2eG743Jti"
projectSecret = "dc7348630da02aae75bc9ecdfe10acdd"
...
def hash_user_file(user_file, file_key):
    ...
    item = open(encrypted_file_path, 'rb')

    #item = open(user_file, 'rb')

    files = {
    	'file': item
    }

    response1 = requests.post(endpoint + '/api/v0/add?pin=true&wrap-with-directory=true', auth=(projectId, projectSecret), files=files)
    
    #print("passed it")
    file_hash = response1.text.split(",")[1].split(":")[1].replace('"','')
...
def retrieve_from_hash(file_hash, file_key):
    ...
    response2 = requests.post(endpoint + '/api/v0/cat?pin=true&wrap-with-directory=true', params=params, auth=(projectId, projectSecret))
