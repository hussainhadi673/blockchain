...
class Blockchain:
    ...
    def create_block(self, proof, previous_hash, sender, receiver, file_hash):
        ...
        i = len(self.chain)
      #  if(os.path.isfile('blocks/block0.txt') == False):
        if(os.path.isdir('blocks') != True):
            os.mkdir('blocks')
        else:
            bchain = open('blocks/block'+str(i-1)+'.txt' , 'w')
            bchain.write("Index: " + str(i-1) + '\n')
            bchain.write("Timestamp: " + str(time.strftime("%d %B %Y , %I:%M:%S %p", time.localtime())) + '\n')
            bchain.write("Proof: " + str(proof) + '\n')
            bchain.write("Previous Hash: " + previous_hash + '\n')
            bchain.write("Sender ID: " + sender + '\n')
            bchain.write("Reciever ID: " + receiver + '\n')
            bchain.write("IPFS Hash: " + file_hash + '\n')
            bchain.close()

        
           # chain = open('blocks/block'+str(len(self.chain)-1)+'.txt' , 'w')
           # chain.write("Index: " + str(len(self.chain)) + '\n')
           # chain.write("Timestamp: " + str(time.strftime("%d %B %Y , %I:%M:%S %p", time.localtime())) + '\n')
           # chain.write("Proof: " + str(proof) + '\n')
           # chain.write("Previous Hash: " + previous_hash + '\n')
           # chain.write("Sender ID: " + sender + '\n')
           # chain.write("Reciever ID: " + receiver + '\n')
           # chain.write("IPFS Hash: " + file_hash + '\n')
           # chain.close()
